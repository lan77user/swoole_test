<?php
    ...
    //  本地  这个代码放在香橙虚拟机/data/weiyan_test文件夹
    include '/data/weiyan_test/PDOconnection.class.php';   //  香橙的服务器
    ...
    class Server{
        ...
        public function onReceive($server, $fd, $reactor_id, $data) {
            ...
            $colon = strpos($data, ':');           //  根据冒号判断是监测还是控制设备发来的命令
            ...
            if($colon > 0) {       //  控制设备
                ...
                if($check){
                    ...
                    if(substr($data, 0,1) == 'D') {
                        ...
                        $this -> exec('INSERT INTO nb_device_test_log (device_sn,device_type,command,status,remark) VALUES ("' . $device_sn . '","2","' . $data . '","8","控制设备发来的错误命令")');
                    }
                ...
                $monitorCheck = $this -> monitorCheck($data,$dataList);   //  监测设备格式分解
                $this -> monitor($fd,$data,$dataList);
            }
        }
...
// ---------------------------------------------------------监测设备开始---------------------------------------------
        ...
        // 监测设备格式分解
        public function monitor($fd,$data = '',$dataList = []) {
            // db::connect();
            // $data = 'D17081TEST001,stream=nongbo,data=02050000000000005f000000de5a0401a501,seq=2003' . "\n" . 'n=SP,seq=2001,state=6' . "\n";
            // $data = 'D17081TEST001,stream=nongbo,data=02050000000000005f000000de5a0401a501,seq=2001' . "\n";
            if(!empty($data) && !empty($dataList)) {

                //  记录设备发来的命令
                db::exec('INSERT INTO nb_monitor_log (device_sn,command,seq,remark) VALUES ("' . $dataList['device_sn'] . '","' . $data . '","' . $dataList['seq'] . '","监测设备发来的正常命令")');
                ...
                //  记录发给设备的命令
                db::exec('INSERT INTO nb_monitor_log (device_sn,command,seq,remark) VALUES ("' . $dataList['device_sn'] . '","' . $command . '","' . $dataList['seq'] . '","发送给设备的命令")');
            }else{
                $arr = explode(',', $data);
                $device_sn = empty(substr($arr[0], 1)) ? '' : substr($arr[0], 1);
                db::exec('INSERT INTO nb_monitor_log (device_sn,command,remark) VALUES ("' . $device_sn . '","' . $data . '","监测设备发来的错误命令")');
        ...
        public function createMonitorCommand($dataList){
            ...
            if(isset($dataList['reply'])) {  //  这里有回复上一条命令
                ...
                if($state == 0){ 
                    ...
                    //  生成seq
                    self::redisInstance() -> incr('M' . $dataList['device_sn'] . '_' . $n);
                    $seq = $this -> redisInstance() -> get('M' . $dataList['device_sn'] . '_' . $n);

                    // 生成控制命令
                    $control = 'C' . $dataList['device_sn'] . ':seq=' . $seq . ',n=' . $n . ',a=' . $a;

                    $reply =  '$ack@' . $dataList['device_sn'] . ':$E=nongbo,$seq=' . $dataList['seq'] . "\n";

                    $command = $control . $reply;

                    //  判断回复的是哪个seq的命令，并修改数据库状态为已执行
                    $res = $this -> getCommandBySeq($dataList['reply']['seq']);
                    if($res[0]['status'] == 0){
                        db::exec('UPDATE nb_send_command SET status = 1 WHERE seq = "' . $dataList['reply']['seq'] . '" AND id = ' . $res[0]['id']);
                    }
                    ...
                    db::exec('UPDATE nb_send_command SET status = ' . $state . ' WHERE seq = "' . $dataList['reply']['seq'] . '" AND control_type = "' . $dataList['reply']['n'] . '" AND device_sn = "' . $dataList['device_sn'] . '"');
                ...
                // 生成控制命令
                $control = 'C' . $dataList['device_sn'] . ':seq=' . $seq . ',n=' . $n . ',a=' . $a;
                $same = db::query('SELECT * FROM nb_send_command WHERE control_type = "' . $n . '" AND device_sn = "' . $dataList['device_sn'] . '" AND status = 0');
                
                if(count($same) <= 0 || $same == false){   //  没有此命令，则插入一条
                    db::query('INSERT INTO nb_send_command (device_sn,command,seq,control_type) VALUES ("' . $dataList['device_sn'] . '","' . $control . '","' . $dataList['seq'] . '","' . $n . '")');
                }else{                   //  否则seq取数据库里未执行成功的seq
                    $control = 'C' . $dataList['device_sn'] . ':seq=' . $same[0]['seq'] . ',n=' . $n . ',a=' . $a;
                    self::redisInstance() -> decr('M' . $dataList['device_sn'] . '_' . $n);
                }
...
// --------------------------------------------------------控制设备开始-------------------------------------------------------------
        ...
        public function controlDevice($fd,$data) {
            ...
            $this -> exec('INSERT INTO nb_device_test_log (device_sn,device_type,command,status,remark) VALUES ("' . $device_sn . '","2","' . $data .  '","8","控制设备设备登录")');
            $this -> fetchNewTCcommand($command,$seq);
            $seq = substr($seq, 0,4);
            $command = $command . ',' . $seq . "\n";
            db::query('INSERT INTO nb_send_command (device_sn,command,seq) VALUES ("' . $device_sn . '","' . $command . '","' . $seq . '")');
            $this -> server -> send($fd, $command);
        ...
        public function receiveCommand($fd,$data,$dataList){
            ...
            if(self::isTCcommandSuitable($command,$seq)){
                ...
                $this -> exec('INSERT INTO nb_device_test_log (device_sn,device_type,command,control_name,arg,status,used_time,remark) VALUES ("' . $device_sn . '","2","' . $command . '","' . $commandType . '","' . $arg . '","' . $replyState . '","' . $usedTime . '","控制设备设备正常回复")');
            }else{
                $this -> exec('INSERT INTO nb_device_test_log (device_sn,device_type,command,control_name,arg,status,remark) VALUES ("' . $device_sn . '","2","' . $data . '","' . $commandType . '","' . $arg . '","' . $replyState . '","控制设备设备主动回复")');
            }
        }
        ...
        public function Close($server,$fd,$reactorId) {
            ...
            if($reactorId >= 0) {
                ...
                if(self::get('REPLY_' . $fd) == false){
                    ...
                    $value = rtrim($value,',');
                    $this -> exec('INSERT INTO nb_device_test_log (device_sn,device_type,command,control_name,arg,status,remark) VALUES ' . $value);
                }else{     
                    //  回复完命令后设备主动断开
                    $this -> del('REPLY_' . $fd);
                    $this -> exec('INSERT INTO nb_device_test_log (device_sn,device_type,status,remark) VALUES ("' . $device_sn . '","2","8","设备已回复命令,主动断开")');
                }
            } 
